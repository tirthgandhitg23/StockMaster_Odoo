...
interface CreateProductBody {
    ...
    unitPrice: number;
    reorderPoint: number;
    // Initial stock can be zero, or we can assume it's added via a transaction later
}
 ...
 */
export const createProduct = async (req: Request<{}, {}, CreateProductBody>, res: Response): Promise<void> => {
    const { sku, name, description, category, unitPrice, reorderPoint } = req.body;

    if (!sku || !name || !category || !unitPrice) {
        res.status(400).json({ message: 'Missing required fields: SKU, Name, Category, and Unit Price.' });
        return;
    }
    ...
    try {
        ...
        const newProduct = new Product({
            ...
            unitPrice,
            reorderPoint,
            stockLocations: [] // Initialize with an empty stock array
        });
 ...
 */
export const createTransaction = async (req: Request<{}, {}, NewTransactionRequestBody>, res: Response): Promise<void> => {
    ...
    switch (type) {
        ...
        case 'TRANSFER':
            ...
            res.status(400).json({ message: 'TRANSFER type requires specific multi-step logic not yet implemented.' });
            return;
        default:
    ...
    try {
        ...
        const stockLocation = product.stockLocations.find(loc => loc.warehouseId.toString() === warehouseId);
        ...
        if (!stockLocation) {
            ...
            await Product.findByIdAndUpdate(productId, {
                $addToSet: { // $addToSet prevents adding duplicates if it somehow already existed
                    stockLocations: {
